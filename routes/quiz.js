  ...
  for(let recno = 0; recno < questionItems.length ; recno++)
    {
      if(questionStyles[recno]==10){
        ...
        //result += "<div><a href='#Question_"+ (recno + 2) + "'><button style='answerBtn btn btn-success'>Next</button></a></div>"
        result += "</div>"
  ...
  getQuestions: (number, setQ) => {
    //=================================
          ...
          function getSingleQuestion(number, style, layout){
            ...
            ////=====================================================================================          
            if(style==3) {
              adj=1; //  Alpha Quiz is easy we don't allow very close letters
            }
    ...
    for(let x=1; x<=number; x++){
      ...
      let limit = "";
      /* ==== This is for advanced features soon!!!
      for(let loop=0; loop<delimiterStyle; loop++){
        for(let times=0; times<=getRandom(0,1); times++)
        {
          const a1 = getRandom(0, 1);
          if(a1==0)
          {
            limit += questionAlphabet(getRandom(0,26) )
          }
          else
          {
            limit +=  String.fromCharCode(getRandom(49,57) );
          }
        }
      }
      */
    ...
    for(let x=0; x<Questions.length; x++) {
      ...
      //let answerValue = q.split("~")[rows-1]
    }
